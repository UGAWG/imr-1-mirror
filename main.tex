...
The Git version control system~\cite{pro-git} and the GitHub website have become
...
only insertion and collapse (EPIC-IC); insertion, collapse, and element/face connectivity swapping (EPIC-ICS);
insertion, collapse, swapping, and node movement (EPIC-ICSM).
EPIC is generally executed using swapping, as it significantly improves quality for a moderate cost increase.
Node movement is generally not used as it is significantly more expensive computationally in its current implementation.
Internally EPIC measures of element quality use a variation of Equation \ref{eq:quality}, moderately decreasing the reported element qualities.
To improve the edge length calculation between unaligned anisotropic metrics, the edge length in EPIC is computed with a numerical integration of the metric field along an edge.
For consistency, EPIC has been modified for this work to use the assumed metric variation as shown in Equation \ref{eq:length}.

\subsection{refine}

Adaptive grid results from two versions of the refine tool are presented.
...
that uses edge swapping and edge collapsing to satisfy quality.
Snapping to geometry (using EGADS) is part of the second (quality) loop:
a step is added, which moves all nodes as far as they can toward
...
For all results presented, the minimum quality threshold was set to 0.3
...
edge/face swapping and refinement (edge splitting) first optimize the
...
quality. The element internal quality function that is optimized is
...
then, the enthusiasm for hybrid parallelism has waned on the solver side,
...
It is based on a two-step procedure to generate a unit-mesh~\cite{loseille-lohner-adapt-boom,loseille-handbook}.
...
Feflo.a can handle nonmanifold surface and/or volume meshes composed of simplicial elements.
...
the same element count (50K), while EPIC-ICSM (47k) and feflo.a (45k)
get slightly better element counts.
...
EPIC-IC     & $0.07$&       $0.16$&       $2.57$&   $ 50977$\\
EPIC-ICS    & $0.26$&       $0.33$&       $2.11$&   $ 49381$\\
EPIC-ICSM   & $0.35$&       $0.32$&       $1.95$&   $ 47545$\\
...
Fig.~\ref{fig:cube-linear-lengths} shows a more in-depth look
...
for EPIC-IC, EPIC-ICS, and Omega\_h showing distinct local maxima.
...
found in Fig.~\ref{fig:cube-linear-lengths-refine-one}:
...
\caption{\footnotesize EPIC-ICSM.}
\end{subfigure}
\begin{subfigure}{.12\textwidth}
\centering
...
On this geometry, we start to see EPIC-IC perform
worse than other codes, with the minimum quality being
below the output precision of our measurement tools.
By adding mesh operations, EPIC-ICS and EPIC-ICSM perform better
with edge length ranges similar to their cube results.
Omega\_h maintains its minimum quality at 0.3, and
...
feflo.a and refine/two also suffer from low minimum element qualities albeit with better edge length conformity.
...
\begin{tabular}{lrrrr}
Code & Min. Quality & Min. Length & Max. Length & \#Elements\\
EPIC-IC    & $<0.01$&       $0.18$&      $ 3.69$&    $40887$\\
EPIC-ICS   &  $0.14$&       $0.31$&      $ 2.62$&    $39935$\\
EPIC-ICSM  &  $0.20$&       $0.34$&      $ 2.44$&    $38302$\\
...
Fig.~\ref{fig:cube-cylinder-linear-lengths} presents edge lengths
as histograms.
EPIC-IC shows two local maxima.
Both EPIC-ICS and feflo.a show a local maximum at the lower edge length bound of $0.707$.
At the extrema, Pragmatic has noticeable
percentages of its edges in the very low range of $[0,0.25]$
while all the EPIC codes show a tail of edges
in the high range $>1.5$.
\begin{figure}
\begin{center}
\begin{subfigure}{.12\textwidth}
\centering
...
are shown in Fig.~\ref{fig:cube-cylinder-linear-qualities}.
The histograms of EPIC-ICS, EPIC-ICSM, and Omega\_h all look fairly
similar, which taper until no significant percentage of elements can be
seen for $Q<0.3$ and no low quality cells $Q<0.1$.
feflo.a and refine/two have similar profiles, but allow a small percentage of elements with quality as low as $Q=0.04$.
Pragmatic has more high quality elements, but also a tail of low-quality
elements that continue down to the lowest levels.
The EPIC-IC has a wider quality distribution than any of the others and a noticeable percentage of low quality, $Q<0.2$, elements which could not be corrected by its limited set of operators.
EPIC-ICS and EPIC-ICSM correct this behavior, suggesting that swapping is the key shape-correction operator.
\begin{figure}
\begin{center}
\begin{subfigure}{.12\textwidth}
\centering
...
\begin{subfigure}{.24\textwidth}
\centering
\includegraphics[width=\textwidth]{refine-two-cube-cylinder-polar-1.png}
\caption{refine/two.}
\label{fig:refine-two-cube-cylinder-polar-1-mesh}
\end{subfigure}
...
all elements above 0.3 quality, so for this metric Omega\_h
preprocessed the metric using gradation control~\cite{Borouchaki-1998-gradation,alauzet-fead-2010-size-gradation-aniso}.
The resulting mesh is shown as the result in Fig.~\ref{fig:omega_h-cube-cylinder-polar-1-mesh}.
...
metric, hence it technically gets a minimum quality result of 0.14 here.
EPIC-IC continues to show poor performance, while EPIC-ICS
and EPIC-ICSM are second only to Omega\_h.
EPIC-ICS and EPIC-ICSM have maximum lengths that are approximately
50\% larger than they were in the linear cube-cylinder case in
...
The curved surface meshes of feflo.a and refine/two are clearly too coarse
(see Fig.~\ref{fig:fefloa-cube-cylinder-polar-1-mesh} and Fig.~\ref{fig:refine-two-cube-cylinder-polar-1-mesh}),
...
\begin{tabular}{lrrrr}
Code & Min. Quality & Min. Length & Max. Length & \#Elements\\
EPIC-IC    &$ <0.01$&      $ 0.12$&      $ 5.29$&    $32775$\\
EPIC-ICS   &$  0.05$&      $ 0.11$&      $ 3.37$&    $31494$\\
EPIC-ICSM  &$  0.10$&      $ 0.16$&      $ 3.14$&    $30378$\\
Omega\_h   &$  0.14$&      $ 0.11$&      $ 1.71$&    $52235$\\
Pragmatic  &$  0.01$&      $ 0.02$&      $ 1.74$&    $33629$\\
feflo.a    &$  0.01$&      $ 0.18$&      $17.40$&    $35310$\\
refine/two &$ <0.01$&      $ 0.07$&      $ 9.35$&    $43613$\\
\end{tabular}
\end{table}

Length histograms for the Polar-1 metric in Fig.~\ref{fig:cube-cylinder-polar-1-lengths}
show similar patterns for the EPIC codes, with the main difference being the
reduction of small and large edges as operators are added, and the centering of the peak closer to 1.0 for EPIC-ICSM.
Omega\_h and Pragmatic show similar and better controlled distributions, consistent
with their maximum edge length results.
feflo.a and refine/two still show a good distributions, illustrating how histograms alone
...
\begin{subfigure}{.12\textwidth}
\centering
\includegraphics[width=\textwidth]{refine-two-cube-cylinder-polar-1-length.pdf}
\caption{refine/two.}
\end{subfigure}
...
\end{center}
\end{figure}
In the quality histograms (Fig.~\ref{fig:cube-cylinder-polar-1-qualities}),
With the exception of Omega\_h, all the codes have a more or less linear quality distribution up to an element quality of 0.5, then a peak distribution at $Q\sim0.8$.
EPIC-IC, feflo.a, and refine/two all have large tails of poor quality elements, and a number of very low quality elements.
Pragmatic decreases the height of the tail for $Q<0.2$.
EPIC-ICS and EPIC-ICSM improve the minimum quality, while maintaining the same overall shape distribution.
...
\begin{subfigure}{.12\textwidth}
\centering
\includegraphics[width=\textwidth]{refine-two-cube-cylinder-polar-1-quality.pdf}
\caption{refine/two.}
\end{subfigure}
...
Omega\_h is able to attain its threshold quality of 0.3, the best in the group.
Finally, note that if one compares the renderings
in Fig.~\ref{fig:cube-cylinder-polar-1-meshes} to those in
Fig.~\ref{fig:cube-cylinder-polar-2-meshes}, then feflo.a 
and refine/two are now much more in agreement with
the other codes.
...
\begin{subfigure}{.24\textwidth}
\centering
\includegraphics[width=\textwidth]{refine-two-cube-cylinder-polar-2.png}
\caption{refine/two.}
\end{subfigure}
...
\begin{tabular}{lrrrr}
Code & Min. Quality & Min. Length & Max. Length & \#Elements\\
EPIC-IC    &$ <0.01$&     $  0.10$&      $ 4.91$&    $47266$\\
EPIC-ICS   &$  0.20$&     $  0.33$&      $ 2.34$&    $46232$\\
EPIC-ICSM  &$  0.24$&     $  0.37$&      $ 2.30$&    $44280$\\
Omega\_h   &$  0.30$&     $  0.24$&      $ 1.81$&    $49151$\\
Pragmatic  &$  0.05$&     $  0.12$&      $ 1.73$&    $47203$\\
feflo.a    &$  0.06$&     $  0.18$&      $ 2.65$&    $53117$\\
refine/two &$  0.01$&     $  0.01$&      $ 3.09$&    $49060$\\
\end{tabular}
\end{table}

The length histograms for the Polar-2 metric were largely the
same as for the Polar-1 metric
(Fig.~\ref{fig:cube-cylinder-polar-1-lengths}),
and we omit them for brevity.
The quality histograms, on the other hand, show very significant differences
between the Polar-1 metric in Fig.~\ref{fig:cube-cylinder-polar-1-qualities}
and the Polar-2 metric in Fig.~\ref{fig:cube-cylinder-polar-2-qualities}.
...
it had in Fig.~\ref{fig:omega_h-cube-cylinder-polar-1-quality}.
...
\begin{subfigure}{.12\textwidth}
\centering
\includegraphics[width=\textwidth]{refine-two-cube-cylinder-polar-2-quality.pdf}
\caption{refine/two.}
\end{subfigure}
...
the solution are updated to reach {\it an optimal} mesh solution
coupled with a desired level of accuracy.
...
depend on the geometry. Providing high quality surface mesh becomes a mandatory feature.
...
interacts with other anisotropic features.
...
We intend to revisit all the database test cases in a parallel environment.
In particular,
...
The polar metrics illustrate how metric gradation can make the metric difficult to satisfy,
