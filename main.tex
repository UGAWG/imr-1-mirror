...
A central repository for the UGAWG has been established on GitHub at https://github.com/UGAWG.   This site houses the data necessary to set up and run benchmark test cases as well as a collection of results from participant adaptation tools.  The adapt-benchmarks section of the respostory has been organzied such that new benchmark cases can easily be added as new test cases are developed.   The adapt-benchmarks section is organzied by geometry model with a separate page available for each model.   Current benchmark geometry models include a Cube and Cube-Cylinder.
...
and hybridisation has been dropped in favor of a more classic purely distributed memory approach in Pragmatic. 
However, some initial design choices were driven
by this approach and still have an influence on the code. 
...
is in progress, an ad-hoc procedure is used in
this paper, that projects new vertices onto an analytic surface 
as soon as they are created.

Note that in the polar cube-cylinder case {\color{red}[TODO ref]}, a coarser metric
is initially prescribed (with a bigger $h_0$) and is progressively modified to reach
the desired metric (by dividing $h_0$ by two at each Pragmatic call).
